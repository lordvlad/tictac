...
/**
 * 8-connected Dijkstra reachability from `origin`.
 * Returns a Float32Array of AP costs from origin to every tile on the grid.
 * Unreachable tiles are set to Infinity.
 */
export function computeReachability(
  ...
  const originIdx = grid.index(origin.x, origin.y)
  costs[originIdx] = 0
  ...
  while (queue.length > 0) {
    ...
    let bestIdx = 0
    let bestCost = costs[queue[0]!]!
    for (let i = 1; i < queue.length; i++) {
      const c = costs[queue[i]!]!
      if (c < bestCost) {
        bestCost = c
        bestIdx = i
      }
    ...
    const currIdx = queue[bestIdx]!
    queue.splice(bestIdx, 1)

    const cx = currIdx % size
    const cy = (currIdx / size) | 0
    const currCost = costs[currIdx]!
    ...
    for (const [dx, dy] of NEIGHBOURS) {
      ...
      // Blocked by another unit (unless it's the origin tile)
      if (nIdx !== originIdx && occupiedTiles.has(nIdx)) continue

      const isDiagonal = dx !== 0 && dy !== 0
      // Strict corner-cutting prevention: both adjacent orthogonal tiles must be walkable
      if (isDiagonal) {
      ...
      if (newCost <= maxCost && newCost < costs[nIdx]!) {
        costs[nIdx] = newCost
