...
/**
 * Fire on the ground: what catches, how long it burns, and where it goes.
 *
...
/**
 * A handover's worth of fire, from the match's dice.
 *
 * Every tile burning at the start spreads first: for each of its orthogonal
 * neighbours that can burn, is not already alight or ash, and that fire can
 * pass to, one draw against the neighbour's flammability. Tiles are taken in
 * index order and neighbours in {@link ORTHOGONAL} order, so both peers draw
 * the same numbers for the same questions. Then every tile that was burning
 * burns down by one, and a tile at nothing has burned out. What caught this
 * handover starts burning next.
 */
