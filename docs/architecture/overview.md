...
4. **Sender-Resolved Combat**: Attacking peer calculates hit/crit/damage rolls locally, packages results into `WireHit`, and sends them to the remote peer for replay. Accurate today, and **scheduled for replacement** by `ITEM-023`: the agreed destination is intent across the wire with both sides recomputing, which makes a match an event log rather than a stream of asserted outcomes. See [RFC-0001](../design/rfc/0001-referee-and-transports.md).
